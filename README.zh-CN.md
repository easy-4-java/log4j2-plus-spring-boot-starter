...
| JDK | `17` |
| 坐标 | `io.github.easy4j:log4j2-plus-spring-boot-starter:3.4.x.20260527-SNAPSHOT` |
| 配置前缀 | `log4j2.plus` |
...
| 属性绑定 | ✅ 稳定 | 绑定 `log4j2.plus.*` 到 `Log4jJdbcProperties` |
| `Log4jJdbcAppenderTemplate` Bean | ✅ 稳定 | 通过 Log4jJdbcAutoConfiguration 自动注册 |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `log4j2.plus.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `log4j2.plus.enabled=true` 与 classpath |
