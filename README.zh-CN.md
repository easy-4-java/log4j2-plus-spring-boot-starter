...
**Spring Boot Starter，集成 log4j2-plus**
...
**log4j2-plus-spring-boot-starter** 是一个面向 使用 log4j2-plus 的应用 的 Spring Boot Starter，用于将 **log4j2-plus** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 log4j2-plus 的全部能力。

| 维度 | 说明 |
|---|---|
| 类型 | Spring Boot Starter |
| 消费方 | 使用 log4j2-plus 的 Spring Boot 应用 |
| 核心能力 | 自动装配、属性绑定、开箱即用的 log4j2-plus Bean |
| JDK | `17` |
| 坐标 | `io.github.easy4j:log4j2-plus-spring-boot-starter:1.0.6.RELEASE` |
| 配置前缀 | `sharding.jdbc.config` |

## 2. 核心能力

| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 log4j2-plus 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `sharding.jdbc.config.*` 到 `Log4jJdbcProperties` |
| `Log4jJdbcAppenderTemplate` Bean | ✅ 稳定 | 通过 Log4jJdbcAutoConfiguration, Log4jShardingJdbcAutoConfiguration 自动注册 |
...
| Bean | 条件 | 缺失时行为 |
|---|---|---|
| `Log4jJdbcAppenderTemplate` | classpath + property | 不创建 |
| `Log4jJdbcInitApplicationListener` | classpath + property | 不创建 |
| `Log4jJdbcAppenderTemplate` | classpath + property | 不创建 |
| `Log4jJdbcInitApplicationListener` | classpath + property | 不创建 |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `sharding.jdbc.config.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `sharding.jdbc.config.enabled=true` 与 classpath |
...
[返回顶部](#readme-top) · [问题反馈](https://github.com/easy-4-java/log4j2-plus-spring-boot-starter/issues) · [仓库地址](https://github.com/easy-4-java/log4j2-plus-spring-boot-starter)
