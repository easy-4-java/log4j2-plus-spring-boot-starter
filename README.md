...
| JDK | `17` |
| Coordinates | `io.github.easy4j:log4j2-plus-spring-boot-starter:1.0.6.RELEASE` |
| Config Prefix | `sharding.jdbc.config` |
...
| Property Binding | ✅ Stable | Binds `sharding.jdbc.config.*` to `Log4jJdbcProperties` |
| `Log4jJdbcAppenderTemplate` bean | ✅ Stable | Auto-registered via Log4jJdbcAutoConfiguration, Log4jShardingJdbcAutoConfiguration |
...
| `Log4jJdbcInitApplicationListener` | classpath + property | not created |
| `Log4jJdbcAppenderTemplate` | classpath + property | not created |
| `Log4jJdbcInitApplicationListener` | classpath + property | not created |
...
|---|---|---|:---:|---|:---:|
| `sharding.jdbc.config.enabled` | boolean | `true` | No | Enable the starter | No |
...
|---|---|---|
| Bean not created | Check auto-configuration report | Verify `sharding.jdbc.config.enabled=true` and classpath |
